"""
Positional Hex encodings (arXiv:2301.07345) — official Q-sage entry point.

``pg`` (path-based / LN) is implemented in ``qsage.encode.paper`` (no
``legacy/`` imports) and matches paper goldens.

``cp`` and ``ibign`` still use the reference tree under ``legacy/`` until
ported the same way.
"""
...
def _args_pg(problem: Path, work: Path) -> SimpleNamespace:
...
def _encode_pg(problem: Path) -> str:
    from qsage.encode.paper.parse.parser import Parse
    from qsage.encode.paper.path_based import PathBasedGoal
    from qsage.encode.paper.qcir_io import encoding_to_qcir as paper_qcir

    prev = Path.cwd()
    try:
        os.chdir(_REPO)
        with tempfile.TemporaryDirectory(prefix="qsage_pos_pg_") as td:
            work = Path(td)
            args = _args_pg(problem, work)
            parsed = Parse(args)
...
def _legacy_args(problem: Path, encoding: str, work: Path) -> SimpleNamespace:
    """Defaults for cp / ibign (still via legacy until ported)."""
...
def _encode_legacy_positional(problem: Path, encoding: str) -> str:
...
def encode_positional(problem: str | Path, encoding: str = "pg") -> str:
    ...
    if encoding == "pg":
        return _encode_pg(problem)
    return _encode_legacy_positional(problem, encoding)
